...
func perform_ai_movement() -> void :
	...
	if player.has_ball() :
		...
		if total_steering_force.length_squared() < 1 :
			if is_ball_possessed_by_oppenent() :
...
func get_assist_formation_steering_force() -> Vector2 :
	...
	var height := get_bicircular_weight(player.position, assist_destination, 30 , 0.2, 60, 1)
...
func has_teammate_in_view() -> bool :
	...
	return players_in_view.find_custom(func(p : Player) :
		return p != player and p.country == player.country
	)
